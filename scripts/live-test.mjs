...
async function call(name, args) {
  const r = await rpc({ jsonrpc: "2.0", id: Math.floor(Math.random() * 1e9), method: "tools/call", params: { name, arguments: args } });
  // A JSON-RPC top-level error, a missing result, or an unparseable body (e.g. a
  // 429/500 plain-text response) are all failures — not silent successes.
  if (!r || r.error || !r.result) {
    return { err: true, text: r?.error ? JSON.stringify(r.error) : "no result / unparseable response", json: undefined };
  }
  const res = r.result;
  const text = res?.content?.[0]?.text ?? "";
  let json;
  try { json = JSON.parse(text); } catch { }
  return { err: res?.isError === true, text, json };
}
...
if (viewId) {
  // try/finally guarantees cleanup even if a check or rpc() throws mid-cycle —
  // a live test must never leak a LiveTest_* table into the real org.
  try {
